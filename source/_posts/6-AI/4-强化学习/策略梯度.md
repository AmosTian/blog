...
假设在策略 $\pi$ 下生成了起始状态为 $s_0$ 的轨迹，奖励为 $(R_{t+1},R_{t+2},\cdots,R_{t+n})$ ，这个轨迹的平均单步奖励为
$$
\begin{aligned}
\overline{r}_{\pi}&=\lim\limits_{n\rightarrow \infty}\frac{1}{n}E\left[R_{t+1}+R_{t+2}+\cdots+R_{t+n}\vert S_t=s_0\right]\\
&=\lim\limits_{n\rightarrow \infty}\frac{1}{n}E\left[\sum\limits_{k=1}^nR_{t+k}\bigg\vert S_t=s_0\right]\\
&\iff\sum\limits_{s}d_{\pi}(s)r_{\pi}(s)
\end{aligned}
$$
...
#### $\beta_t=\frac{Q_{\pi}(s_t,a_t)}{\pi(a_t\vert s_t,\theta^{(t)})}$ 能很好平衡探索与利用
