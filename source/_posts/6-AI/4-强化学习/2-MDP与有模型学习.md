...
智能体在选择动作时，会更多地考虑那些能够带来长期利益的行动，而不是追求短期的高回报。但有些马尔可夫过程是带环的，我们想量化状态价值就必须避免无穷奖励
...
$$
\mathbf{V}^{(t+1)}_{\pi}=\mathbf{R}_{\pi}+\gamma \mathbf{P}_{\pi}\mathbf{V}^{(t)}
$$
...
DP解决预测问题是一个 **同步迭代** 过程：将贝尔曼期望备份(Bellman expection backup) 变为迭代过程，反复迭代直到收敛
...
$$
\begin{aligned}
V(s)&=\max\limits_{\pi}\sum\limits_{a}\pi(a\vert s)\left(\sum\limits_{r'}P(r'\vert s,a)r'+\gamma \sum\limits_{s'}P(s'\vert s,a)\underbrace{V(s')}_{随机初始值，迭代至收敛，相当于已知}\right)\\
&=\max\limits_{\pi}\sum\limits_{a}\pi(a\vert s)Q(s,a),\forall s\in \mathcal{S}
\end{aligned}
$$
...
> $$
> \mathbf{V}^{(t+1)}=f(\mathbf{V}^{(t)})=\max\limits_{\pi\in\Pi}(\mathbf{R}_{\pi}+_\gamma P_{\pi}\mathbf{V}^{(t)}),t=0,1,2,\cdots
> $$
...
2. 对于任意的 $a\in \mathcal{A}$ ，计算
   $$
   Q^{(t)}(s,a)=\sum\limits_{r'}P(r'\vert s,a)r'+\gamma\sum\limits_{s'}P(s'\vert s,a)\mathbf{V}^{(t)}(s')
   $$

3. 通过贪心策略计算对于每个状态的最佳策略 $\pi^{(t+1)}(a\vert s)$ 
   $$
   ...
   \end{cases}
   $$
   由于采取确定性策略，$\pi(a^*\vert s)=1,\pi(a^{others}\vert s)=0$ ，所以  $V^{(t+1)}(s)=\max\limits_{a}Q^{(t)}(s,a)$ 
...
$$
\begin{aligned}
V^{(t+1)}(s)&=f(V^{(t)})=\max\limits_{\pi}\left(\mathbf{R}_{\pi}+\gamma P_{\pi}\mathbf{V}^{(t)}\right),k=0,1,2,3,\cdots\\
...
1. 第 $t$ 轮迭代，$t=0,1,2,\cdots$：

   计算  $Q^{(t)}$ 
   $$
   Q^{(t)}(s,a)=R(s,a)+\gamma \sum\limits_{s'\in S}P(s'\vert s,a)\cdot V^{(t)}(s')\\
   $$
   ![image-20240123174813238](2-MDP与有模型学习/image-20240123174813238.png)

   策略更新
   $$
   \pi^{(t+1)}(s)=\mathop{\mathrm{argmax}}\limits_{\pi}\sum\limits_{a}\pi(a\vert s)Q^{(t)}(s,a)\\
   \Updownarrow\\
   \pi^{(t+1)}(a\vert s)=\begin{cases}
   1&a=a_{*}^{(t)}\\
   0&a\neq a_{*}^{(t)}
   \end{cases}\quad,a_{*}^{(t)}=\mathop{\mathrm{argmax}}\limits_a\mathbf{Q}^{(t)}(s,a)
   $$

2. 价值更新
   $$
   \begin{aligned}
   V^{(t+1)}(s)&=R_{\pi^{(t+1)}}(s,a)+\gamma \sum\limits_{s'\in S}P_{\pi^{(t+1)}}(s'\vert s,a)\cdot V^{(t)}(s')\\
   &=\sum\limits_{a}\pi^{(t+1)}(a\vert s)\underbrace{\left(\sum\limits_{r'}P(r'\vert s,a)r'+\gamma\sum\limits_{s'}P(s'\vert s,a)V^{(t)}(s')\right)}_{Q^{(t)}(s,a)}
   \end{aligned}
   $$
   由于采用确定性策略，所以最优动作的概率 $\pi(a^*\vert s)=1,\pi(a^{others}\vert s)=0$ 。
   $$
   V^{(t+1)}=\max\limits_{a}Q^{(t)}(a,s)
   $$

在每轮迭代中，通过状态价值更新动作价值，同时更新策略，进而通过自举得到下一轮状态价值，所以核心是状态价值的更新，相当于直接通过价值函数迭代
...
$$
V^{(0)}(s),V^{(t)}(s)\rightarrow Q^{(t)}(s,a)\rightarrow 贪心策略 \pi^{(t+1)}(a\vert s)\rightarrow V^{(t+1)}=\max\limits_{a}Q^{(t)}(s,a)
$$
...
&第t轮迭代,当V^{(t)}未收敛时(\Vert V^{(t)}-V^{(t-1)}\Vert>\varepsilon)\\
&\quad对于 \forall s\in \mathcal{S}:\\
&\qquad 对于 \forall a\in \mathcal{A}(s):\\
&\quad \qquad 计算Q值：Q^{(t)}(s,a)=\sum\limits_{r'}P(r'\vert s,a)r'+\gamma\sum\limits_{s'}P(s'\vert s,a)V^{(t-1)}(s')\\
&\qquad 求最优动作：a_{*}^{(t)}(s)=\mathop{\mathrm{argmax}}\limits_{a}Q^{(t)}(s,a)\\
&\qquad 策略更新：\pi^{(t+1)}(a_{*}^{(t)}\vert s)=1,\pi^{(t+1)}(a^{others}\vert s)=0\\
&\qquad 价值更新：V^{(t+1)}(s)=\max\limits_{a}Q^{(t)}(a,s)\\
\hline
\end{array}
$$
...
1. 策略评估(policy evaluation,PE)：利用贝尔曼公式评估给定策略 $\pi^{(t)}$ 的状态价值
   $$
...
&当策略未收敛，第 t轮迭代:\\
&\quad 策略评估:\\
&\qquad 初始化：一个随机初始化的状态价值\mathbf{V}^{(0)}_{\pi^{(t)}}\\
&\qquad 当\mathbf{V}^{(j)}_{\pi^{(t)}} 未收敛，进行第j+1轮迭代：\\
&\quad\qquad 对每个状态 s\in\mathcal{S}:\\
&\qquad\qquad \mathbf{V}_{\pi^{(t)}}^{(j+1)}(s)=\sum\limits_{a\in A}\pi^{(t)}(a\vert s)\left(\sum\limits_{r'}P(r'\vert s,a)r'+\gamma\sum\limits_{s'\in S}P(s'\vert s,a)\cdot V_{\pi^{(t)}}^{(j)}(s')\right)\\
&\quad 策略改进:\\
...
&\qquad \quad a_*^{(t)}(s)=\mathop{\mathrm{argmax}}\limits_{a}Q_{\pi^{(t)}}(s,a)\\
&\quad \qquad 若a_*^{(t)}(s)==a ,则\pi^{(t+1)}(a\vert s)=1，否则\pi^{(t+1)}(a\vert s)=0\\
\hline
\end{array}
$$
...
若要证明 **策略迭代的迭代轮数 $t$ 小**，只需证明对于任一 $t$ ，有 $\mathbf{V}^{(t)}\le \mathbf{V}_{\pi^{(t)}}\le \mathbf{V}^*$ 
...
\end{aligned}
$$
因此，$\mathbf{V}_{\pi^{(t+1)}}\ge\mathbf{V}^{(t+1)}$ 成立

故可知 $\mathbf{V}^{(t)}\le \mathbf{V}_{\pi^{(t)}}\le \mathbf{V}^*$ ，即策略迭代的收敛速度快
...
&当策略未收敛，第 t轮迭代:\\
&\quad 策略评估:\\
&\qquad 初始化：一个随机初始化的状态价值\mathbf{V}^{(0)}_{\pi^{(t)}}=V_{\pi^{(t-1)}}，设定迭代次数超参数j_{truncate}\\
