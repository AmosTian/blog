...
样本方差：$S^2=\frac{1}{n-1}\sum_\limits{i=1}^n(X_i-\overline{X})^2$ 
...
$$
\left<f(X),f(Y)\right>=f^T(Y)\cdot f(X)=\sum_\limits{i=1}^n\sum_\limits{j=1}^n(x_ix_j)\cdot(y_iy_j)
$$
 时间复杂度为 $O(n^2)$ 
$$
K(X,Y)=(\left<X,Y\right>)^2=\left(\sum_\limits{i=1}^nx_iy_i\right)^2
$$
时间复杂度为 $O(n)$
