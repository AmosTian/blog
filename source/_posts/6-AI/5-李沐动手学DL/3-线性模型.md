...
$$
\mathbf{X}=\begin{bmatrix}\mathbf{x}_1\\\mathbf{x}_2\\\vdots\\\mathbf{x}_n\end{bmatrix}\in\R^{n\times d},\hat{\mathbf{y}}=\mathbf{X}\cdot\mathbf{w}+b=\begin{bmatrix}\mathbf{x}_1\cdot\mathbf{w}+b\\\mathbf{x}_2\cdot\mathbf{w}+b\\\vdots\\\mathbf{x}_n\cdot\mathbf{w}+b\end{bmatrix}=\begin{bmatrix}\hat{y}_1\\\hat{y}_2\\\vdots\\\hat{y}_n\end{bmatrix}\in \R^{n\times1}
$$
定义损失函数：将模型对数据的预测结果与真实结果误差作为损失函数，希望这个损失越小越好
$$
\ell\left(\mathbf{X},\mathbf{y},\mathbf{w},b\right)=\frac{1}{2n}\sum\limits_{i=1}^n\left[y_i-\left(\mathbf{x}_i\cdot\mathbf{w}+b\right)\right]^2=\frac{1}{2n}\Vert \mathbf{y}-\mathbf{X}\cdot\mathbf{w}\Vert^2
$$
...
$$
\hat{y}=(\mathbf{x}_i,\mathbf{w})=[x^{(1)}_i,x^{(2)}_i,\cdots,x^{(d)}_i,1]\begin{bmatrix}w_1\\w_2\\\vdots\\w_d\\b
...
\end{array}
\right]=\begin{bmatrix}x_1^{(1)}&x_1^{(2)}&\cdots&x_1^{(d)}&1\\x_2^{(1)}&x_2^{(2)}&\cdots&x_2^{(d)}&1\\\vdots&\vdots&\ddots&\vdots&\vdots\\x_n^{(1)}&x_n^{(2)}&\cdots&x_n^{(d)}&1\end{bmatrix}\in\R^{n\times (d+1)}\quad,\mathbf{w}=\begin{bmatrix}w_1\\w_2\\\vdots\\w_d\\b\end{bmatrix}\in \R^{(d+1)\times 1}
$$
损失函数为 
$$
\ell\left(\mathbf{X},\mathbf{y},\mathbf{w}\right)=\frac{1}{2n}\sum\limits_{i=1}^n\left[y_i-\left(\mathbf{x}_i\mathbf{w}^T+b\right)\right]^2=\frac{1}{2n}\Vert \mathbf{y}-\mathbf{X}\mathbf{w}\Vert^2
$$
对参数求梯度
$$
\frac{\partial}{\partial \mathbf{w}}\ell(\mathbf{X},\mathbf{y},\mathbf{w})=\frac{1}{n}\left(\mathbf{y}-\mathbf{X}\mathbf{w}\right)^T\mathbf{X}
$$
由于该函数是凸函数，最优解为
$$
\frac{\partial}{\partial \mathbf{w}}\ell(\mathbf{X},\mathbf{y},\mathbf{w})=0\\
\iff \frac{1}{n}\left(\mathbf{y}-\mathbf{X}\mathbf{w}\right)^T\mathbf{X}=0\\
\iff \left(\mathbf{y}^T-\mathbf{w}^T\mathbf{X}^T\right)\mathbf{X}=0\\
\iff\mathbf{y}^T\mathbf{X}=\mathbf{w}^T\mathbf{X}^T\mathbf{X}\\
\mathbf{w}^{*T}=\left(\mathbf{y}^T\mathbf{X}\right)\cdot\left(\mathbf{X}^T\mathbf{X}\right)^{-1}\\
...
$$
\frac{\partial}{\partial \mathbf{w}}\ell(\mathbf{X},\mathbf{y},\mathbf{w})=\frac{1}{\vert \mathcal{B}\vert}\left(\mathbf{y}-\mathbf{X}\mathbf{w}\right)^T\mathbf{X}
$$
...
对类别编码，若有 $K$ 个类别，使用one-hot向量对真实样本 $(\mathbf{x}_i,y_i)$ 进行编码 $\mathbf{y}_i=[y_i^{(1)},y_i^{(2)},\cdots,y_i^{(K)}]$，令 $\kappa$ 为 $\mathbf{x}_i$ 的真实类别，则 $y_i^{(k)}=\begin{cases}1&,k=\kappa\\0&,k\neq \kappa\end{cases},k=1,\cdots,K$ ，样本 $\mathbf{x}_i$ 的真实类别 $y_i^{(k)}$ 服从一个分布 $p$ 
 ...
 $\hat{y}_i^{(k)}=q(\hat{y}_i=k\vert \mathbf{x}_i)=softmax(o_i^{(k)})=\frac{e^{o_i^{(k)}}}{\sum\limits_{k=1}^Ke^{o_i^{(k)}}}=\frac{e^{\mathbf{w}_k^T\cdot \mathbf{x}_i}}{\sum\limits_{k=1}^Ke^{\mathbf{w}_k^T\cdot \mathbf{x}_i}}$ 表示将样本 $\mathbf{x}_i$ 预测为 $k$ 类的置信度，对样本  $\mathbf{x}_i$ 的类别预测置信度服从分布 $q$ 
...
&=-\log\left(\frac{e^{\mathbf{w}_{\kappa}^T\cdot \mathbf{x}_i}}{\sum\limits_{k=1}^Ke^{\mathbf{w}_k^T\cdot \mathbf{x}_i}}\right)=\log\sum\limits_{k=1}^Ke^{\mathbf{w}_k^T\cdot \mathbf{x}_i}-\mathbf{w}_{\kappa}^T\cdot \mathbf{x}_i
\end{aligned}
$$
对参数 $\mathbf{w}_{\kappa}$ 求梯度
$$
\begin{aligned}
\frac{\partial }{\partial \mathbf{w}_{\kappa}}\ell(\mathbf{y}_i,\hat{\mathbf{y}}_i)&=\frac{\partial}{\partial \mathbf{w}_\kappa}\left[\log\left(\cdots+e^{\mathbf{w}_{\kappa}^T\cdot \mathbf{x}_i}+\cdots\right)-\mathbf{w}_{\kappa}^T\cdot \mathbf{x}_i\right]\\
&=\frac{e^{\mathbf{w}_{\kappa}^T\cdot \mathbf{x}_i}\cdot \mathbf{x}_i^T}{\sum\limits_{k=1}^Ke^{\mathbf{w}_k^T\cdot \mathbf{x}_i}}-\mathbf{x}_i^T\\
&=\left(\hat{y}_i^{(\kappa)}-y_i^{(\kappa)}\right)\mathbf{x}_i^T
\end{aligned}
$$
...
- 每个样本展平为 $\mathbf{x}_i\in\R^{1\times 784}$ 的行输入向量，批量样本矩阵表示为 $\mathbf{X}=\begin{bmatrix}\mathbf{x}_1\\\vdots\\\mathbf{x}_{256}\end{bmatrix}\in\R^{256\times 784}$  

净输出：净输出矩阵

- 每个样本的净输出向量 $\mathbf{o}_i\in \R^{1\times 10}$ 列向量，批量净输出矩阵表示为 $\mathbf{O}=\begin{bmatrix}\mathbf{o}_1\\\mathbf{o}_2\\\vdots\\\mathbf{o}_{256}\end{bmatrix}\in \R^{256\times 10}$ 

参数 $\mathbf{w}_k\in \R^{1\times 784},k\in [1,10]$ ，参数矩阵 $\mathbf{W}=[\mathbf{w}_1^T,\mathbf{w}_2^T,\cdots,\mathbf{w}_{10}^T]\in \R^{784\times 10}$ ，偏置 $\mathbf{b}\in \R^{1\times 10}$ 行向量
$$
\begin{aligned}
\mathbf{X}\times \mathbf{W}&=\begin{bmatrix}\mathbf{x}_1\\\mathbf{x}_2\\\vdots\\\mathbf{x}_{256}
...
输出：softmax后的置信度矩阵 / 归一化后的置信度矩阵 $\hat{\mathbf{Y}}=\begin{bmatrix}\hat{\mathbf{y}}_1\\\hat{\mathbf{y}}_2\\\vdots\\\hat{\mathbf{y}}_{256}\end{bmatrix}$ 
...
\end{bmatrix}\\
\mathbf{partition}=\begin{bmatrix}
\sum\limits_{k=1}^K e^{o_1^{(k)}}\\
\sum\limits_{k=1}^K e^{o_2^{(k)}}\\
\vdots\\
\sum\limits_{k=1}^K e^{o_n^{(k)}}\\
\end{bmatrix}\quad ,\hat{\mathbf{y}}_i=softmax(\mathbf{o}_i)=\left[\frac{e^{o_i^{(1)}}}{\sum\limits_{k=1}^{K} e^{o_i^{(k)}}},\frac{e^{o_i^{(2)}}}{\sum\limits_{k=1}^{K} e^{o_i^{(k)}}},\cdots,\frac{e^{o_i^{(K)}}}{\sum\limits_{k=1}^{K} e^{o_i^{(k)}}}\right]=\frac{e^{\mathbf{o}_i}}{partition_i}
$$
...
输入 $\mathbf{x}\in \R^{1\times n}$ 

隐藏层 $\mathbf{W}_1\in \R^{n\times m}$ ，$\mathbf{b}_{1}\in \R^{1\times m}$  

- 隐藏层输出=输出层输入：$\mathbf{h}_1=\sigma(\mathbf{x}\mathbf{W}_1+\mathbf{b}_1)\in \R^{1\times m}$
- $\sigma(\cdot)$ 为按元素非线性激活函数

输出层 $\mathbf{w}_2\in \R^{m\times 1}$ ，$b_2\in \R$

- $\hat{o}=\mathbf{h}_1\mathbf{w}_2+b_2\in\R$ 
...
$$
\begin{aligned}
o&=\mathbf{h}_1\mathbf{w}+b_2\\
&=\left(\mathbf{x}\mathbf{W}_1+\mathbf{b}_1\right)\mathbf{w}+b_2\\
&=\mathbf{x}\mathbf{W}_1\mathbf{w}+b'
\end{aligned}
$$
即仍然是一个线性感知器，这样的学习效果不好
...
输出层 $\mathbf{w}_2\in \R^{m\times k}$ ，$\mathbf{b}_2\in \R^{1\times k}$ 

- $\hat{\mathbf{o}}=\mathbf{h}_1\mathbf{w}_2+\mathbf{b}_2\in\R^{1\times k}$ 
- $\hat{y}=softmax(\hat{o})$ 
...
第一隐藏层 $\mathbf{W}_1\in \R^{n\times m_1}$ ，$\mathbf{b}_{1}\in \R^{1\times m_1}$  

- 第一隐藏层输出=第二隐藏层输入：$\mathbf{h}_1=\sigma(\mathbf{x}\mathbf{W}_1+\mathbf{b}_1)\in \R^{1\times m_1}$
- $\sigma(\cdot)$ 为按元素非线性激活函数

第二隐藏层 $\mathbf{W}_2\in \R^{m_1\times m_2}$ ，$b_2\in \R^{1\times m_2}$

- 第二隐藏层输出=第三隐藏层输入：$\mathbf{h}_2=\sigma(\mathbf{h}_1\mathbf{W}_2+\mathbf{b}_2)\in \R^{1\times m_2}$

第三隐藏层 $\mathbf{W}_3\in \R^{m_2\times m_3}$ ，$b_3\in \R^{1\times m_3}$

- 第三隐藏层输出=输出层输入：$\mathbf{h}_3=\sigma(\mathbf{h}_2\mathbf{W}_3+\mathbf{b}_3)\in \R^{1\times m_3}$

输出层 $\mathbf{w}_4\in \R^{m_3\times k}$ ，$\mathbf{b}_2\in \R^{1\times k}$ 

- $\hat{\mathbf{o}}=\mathbf{h}_3\mathbf{w}_4+\mathbf{b}_2\in\R^{1\times k}$ 
- $\hat{y}=softmax(\hat{o})$ 
...
- 单隐藏层，$m_1$ 设置大一些
- 网络层数深一些，相对单隐藏层的 $m_1$ ，多隐藏层的 $m_1$ 会小一些，$m_1>m_2>m_3$
