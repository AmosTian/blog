...
&=\mathbf{h}_{t-1}+\sigma\left(\mathbf{x}_{t}\mathbf{W}_{hx}^T+\mathbf{b}_h\right)
\end{aligned}
$$
...
&=\mathbf{h}_{t-1}+\sigma\left(\mathbf{x}_{t}\mathbf{W}_{hx}^T+\mathbf{h}_{t-1}\mathbf{W}_{hh}^T+\mathbf{b}_h\right)
\end{aligned}
$$
...
门的输入：当前时间步的输入 $\mathbf{X}_{t}\in \R^{B\times V}$ 和上一时刻隐状态 $\mathbf{H}_{t-1}\in \R^{B\times h}$ ，两个门的经过 *siogmoid* 函数输出

![image-20240407101355042](9-经典循环神经网络/image-20240407101355042.png)

重置门 $\mathbf{R}_{t}\in \R^{B\times h},\mathbf{Z}_{t}\in \R^{B\times h}$ 

$$
\mathbf{R}_t = \sigma\left(\mathbf{X}_{t} \mathbf{W}_{rx}^T + \mathbf{H}_{t-1} \mathbf{W}_{rh}^T + \mathbf{b}_r\right),\\
\mathbf{Z}_t = \sigma\left(\mathbf{X}_{t} \mathbf{W}_{zx}^T + \mathbf{H}_{t-1} \mathbf{W}_{zh}^T + \mathbf{b}_z\right)
$$
其中，$\mathbf{W}_{rx}\in \R^{h\times V},\mathbf{W}_{zx}\in \R^{h\times V}$ ，$\mathbf{W}_{rh},\mathbf{W}_{zh}\in \R^{h\times h}$ ，$\mathbf{b}_r,\mathbf{b}_h\in \R^{1\times h}$ 是可学习的参数

#### 候选隐状态

![image-20240407103134627](9-经典循环神经网络/image-20240407103134627.png)
$$
\tilde{\mathbf{H}}_t=\tanh\left(\mathbf{X}_{t}\mathbf{W}_{hx}^T+\left(\mathbf{R}_t\odot \mathbf{H}_{t-1}\right)\mathbf{W}_{hh}^T+\mathbf{b}_h\right)
$$
更新门，可以控制历史状态对当前状态的影响程度

- 当 $\mathbf{R}_t=\mathbf{I}$ 时，可以保留全部的历史信息，恢复为RNN；
- 当 $\mathbf{R}_t=\mathbf{0}$ 时，丢弃全部的历史信息，从当前时刻恢复为输入 $\mathbf{X}_{t}$ 的状态；

#### 隐状态

![image-20240407103538566](9-经典循环神经网络/image-20240407103538566.png)

$\mathbf{Z}_t$ 确定新的隐状态 $\mathbf{H}_{t}\in \R^{B\times h}$ 在多大程度上来自旧的隐状态 $\mathbf{H}_{t-1}$ 与新的候选状态 $\tilde{\mathbf{H}}_{t}$ 
$$
\mathbf{R}_t = \sigma\left(\mathbf{X}_{t} \mathbf{W}_{rx}^T + \mathbf{H}_{t-1} \mathbf{W}_{rh}^T + \mathbf{b}_r\right),\\
\tilde{\mathbf{H}}=\tanh\left(\mathbf{X}_{t}\mathbf{W}_{hx}^T+\left(\mathbf{R}_t\odot \mathbf{H}_{t-1}\right)\mathbf{W}_{hh}^T+\mathbf{b}_h\right)\\
\mathbf{Z}_t = \sigma\left(\mathbf{X}_{t} \mathbf{W}_{zx}^T + \mathbf{H}_{t-1} \mathbf{W}_{zh}^T + \mathbf{b}_z\right)\\
...
重置门(reset gate)、遗忘门：控制历史隐状态的保留程度，若当前状态比较重要，则下一个时间步尽量不更新当前状态相应的隐状态
...
输入信息：当前时间步的输入 $\mathbf{X}_{t}\in \R^{B\times V}$ 和上一时刻隐状态 $\mathbf{H}_{t-1}\in \R^{B\times h}$ 

![image-20240407160511029](9-经典循环神经网络/image-20240407160511029.png)

输入门 $\mathbf{I}_t\in \R^{B\times h}$ ，遗忘门 $\mathbf{F}_t\in \R^{B\times h}$ ，输出门 $\mathbf{O}_t\in \R^{B\times h}$
$$
\mathbf{I}_t=\sigma\left(\mathbf{X}_{t}\mathbf{W}_{ix}^T+\mathbf{H}_{t-1}\mathbf{W}_{ih}^T+\mathbf{b}_i\right)\\
\mathbf{F}_t=\sigma\left(\mathbf{X}_{t}\mathbf{W}_{fx}^T+\mathbf{H}_{t-1}\mathbf{W}_{fh}^T+\mathbf{b}_f\right)\\
\mathbf{O}_t=\sigma\left(\mathbf{X}_{t}\mathbf{W}_{ox}^T+\mathbf{H}_{t-1}\mathbf{W}_{oh}^T+\mathbf{b}_i\right)
$$
其中，$\mathbf{W}_{ix},\mathbf{W}_{fx},\mathbf{W}_{ox}\in \R^{h\times V}$ ，$\mathbf{W}_{ih},\mathbf{W}_{fh},\mathbf{W}_{oh}\in \R^{h\times h}$ ，$\mathbf{b}_i,\mathbf{b}_f，\mathbf{b}_o\in \R^{1\times h}$ 是可学习参数

#### 候选记忆元

![image-20240407161153837](9-经典循环神经网络/image-20240407161153837.png)

候选记忆元 $\tilde{\mathbf{C}}_{t}\in \R^{B\times h}$ 
$$
\tilde{\mathbf{C}}_{t}=\tanh\left(\mathbf{X}_{t}\mathbf{W}_{cx}^T+\mathbf{H}_{t-1}\mathbf{W}_{ch}^T+\mathbf{b}_c\right)
$$
其中，$\mathbf{W}_{cx}\in\R^{h\times V}$ ，$\mathbf{W}_{ch}\in \R^{h\times h}$ ，$\mathbf{b}_c\in \R^{1\times h}$ 都是可学习的参数
...
- 关于 $\tanh\left(\mathbf{C}_{t}\right)$ 的理解，确保 $\mathbf{H}_{t}$ 压缩在 $[-1,1]$ 
  ...
  $\mathbf{F}_t$ 和 $\mathbf{I}_t$ 都在 $[0,1]$ 区间内，所以 $\mathbf{C}^{(t)}\in [-2,2]$ ，为了将 $\mathbf{H}_{t}$ 恢复到 $[-1,1]$ 内，使用 $\tanh$ 
...
隐变量与观测值之间的关系不仅是单向非线性的，实际上隐变量和观测值与具体的函数形式的交互方式是相当随意的
...
在 $t$ 步有一个小批量的输入数据 $\mathbf{X}_t\in \R^{B\times V}$ ，第 $l$ 个隐藏层的隐状态 $\mathbf{H}^{(l)}_t\in \R^{B\times h}$ ，输出层变量 $\mathbf{O}_t\in \R^{B\times q}$ 。可认为 $\mathbf{H}_t^{(0)}=\mathbf{X}_t$ ，第 $l$ 个隐藏层的隐状态使用的激活函数为 $\phi_l(\cdot)$
$$
\mathbf{H}_t^{(l)}=\phi_l\left(\mathbf{H}_t^{(l-1)}\cdot \mathbf{W}_{hx}^{(l)}+\mathbf{H}^{(l)}_{t}\cdot\mathbf{W}_{hh}^{(l)}+\mathbf{b}^{(l)}_h\right)
$$
其中，

- 当 $l\ge 2$ 时，$\mathbf{W}_{hx}^{(l)}\in \R^{h\times h}$ ，$\mathbf{W}_{hh}^{(l)}\in \R^{h\times h}$ ，$\mathbf{b}_h^{(l)}\in \R^{1\times h}$ 是第 $l$ 层可学习的参数
- 当 $l=1$​ 时，$\mathbf{W}_{hx}^{(l)}\in \R^{h\times V}$ ，$\mathbf{W}_{hh}^{(l)}\in \R^{h\times h}$，$\mathbf{b}_h^{(l)}\in \R^{1\times h}$ 是第 $1$ 层可学习的参数

输出层仅基于第 $L$ 层的隐状态
$$
\mathbf{O}_t=\mathbf{H}_t^{(L)}\cdot\mathbf{W}_{qh}+\mathbf{b}_q
$$
其中，$\mathbf{W}_{qh}^{(L)}\in \R^{q\times h}$ ，$\mathbf{b}_q\in \R^{1\times q}$ 都是输出层可学习的参数
...
`lstm` 循环层：输入神经元数为 $V$ ，各隐藏层神经元数都为 $h$ 
...
在任意时间步 $t$ ，会有一个隐变量 $h_t$ ，每个隐状态都有一定概率 $p(x_t\vert h_t)$ 得到观测 $x_t$。另外，隐状态之间存在状态转移概率 $p(h_{t+1}\vert h_t)$ ，因此获取一个序列 $\{x_1,x_2,\cdots,x_T\}$ 的概率服从一个联合概率
$$
p(x_1,x_2,\cdots,x_T,h_1,h_2,\cdots,h_T)=\prod\limits_{t=1}^Tp(h_{t}\vert h_{t-1})p(x_t\vert h_t)
$$
假设序列缺失 $x_j$ ，变为一个填空任务，则目标函数变为 $\max p(x_j\vert x_{-j})=\max p(x_j\vert x_1,x_2,\cdots,x_{j-1},x_{j+1},\cdots,x_T)$  

隐状态 $h_t$ 是一个与时间有关的变量，若所有隐状态的状态值是有限的，即每个 $h_t$ 都可以有 $k$ 个不同的值，则相邻两个时刻间的状态转移 $p(h_{t+1}\vert h_t)\in \R^{k\times k}$ 是一个状态转移矩阵 ，要得到 $p(x_1,\cdots,x_T)$ 是 $k^T$ 个路径的求和
$$
...
- *前向递归* 
  $$
  \pi_{t}(h_{t}) = \sum_{h_{t-1}} \pi_{t-1}(h_{t-1}) P(x_{t-1} \vert h_{t-1}) P(h_{t} \vert h_{t-1})\\
  \pi_1(h_1)=p(h_1)
  $$

- *后向递归* 
  $$
  \rho_{t}(h_{t})= \sum_{h_{t+1}} P(h_{t+1} \mid h_{t}) P(x_{t+1} \mid h_{t+1}) \rho_{t+1}(h_{t+1})\\
  \rho_T(h_T) = 1
  $$
...
在任意时间步 $t$ ，有小批量数据 $\mathbf{X}_{t}\in \R^{B\times V}$ 。该时间步的前向隐状态 $\overrightarrow{\mathbf{H}}_t\in \R^{B\times h}$  和反向隐状态 $\overleftarrow{\mathbf{H}}_t\in \R^{B\times h}$ 
$$
\begin{aligned}
...
\end{aligned}
$$
其中，权重$\mathbf{W}_{hx}^{(f)} \in \mathbb{R}^{h \times V}, \mathbf{W}_{hh}^{(f)} \in \mathbb{R}^{h \times h}, \mathbf{W}_{hx}^{(b)} \in \mathbb{R}^{h \times V}, \mathbf{W}_{hh}^{(b)} \in \mathbb{R}^{h \times h}$
和偏置$\mathbf{b}_h^{(f)} \in \mathbb{R}^{1 \times h}, \mathbf{b}_h^{(b)} \in \mathbb{R}^{1 \times h}$都是模型的可学习参数
...
https://blog.csdn.net/weixin_42645636/article/details/129022967
...
下载双语句子，“英-法”数据集。数据集的每一行由制表符分割两种语言，前半部分为英语句子后半部分为法语句子。
...
其次，为便于计算，记录每个句子子序列的有效长度（排除 `<pad>` ）

```python
#@save
def build_array_nmt(lines, vocab, num_steps):
    """将机器翻译的句子序列切分为等长的子序列，长度为时序窗口"""
...
$len_{label},len_{pred}$ 分别是标签序列中的词元数和预测序列中的词元数，$k$ 是检测的最大语法单元，即最多检测预测序列中的 $k-gram$ 属于真实序列 $\mathbf{Y}$ 的概率
...
**序列的时序信息存在隐状态中** 时序窗口中，最后一个时间步的隐状态是对这个子序列的通用时序特征的累积描述
