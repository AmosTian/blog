...
| 符号                                       | 含义                             |
| ------------------------------------------ | -------------------------------- |
| $\mathbf{W}_{l}\in \R^{M_l\times M_{l-1}}$ | 第 $l-1$ 层到第 $l$ 层的权重矩阵 |
| $\mathbf{b}_{l}\in \R^{1\times M_l}$       | 第 $l-1$ 层到第 $l$ 层的偏置     |
...
| $\mathbf{h}_{l}\in \R^{1\times M_{l}}$ | 第 $l$ 层神经元的输出（活性值）     |

### 3.2.2 信息传播公式

神经网络的第 $l$ 层有 $M_l$ 个神经元，相应的有 $M_l$ 个净输入和活性值，所以二者需要由 $\R^{1\times M_l}$ 维向量来表示

第 $l$ 层的输入为第 $l-1$ 层的活性值，相应的为 $\R^{1\times M_{l-1}}$ 向量

故第 $l$ 层神经元的净输入需要经过一个 **仿射变换**，即
$$
\begin{aligned}
\mathbf{o}_{l}&=\mathbf{x}_l\mathbf{W}_{l}^{T}+\mathbf{b}^{(l)}，其中 \mathbf{W}_{l}\in \R^{M_l\times M_{l-1}}\\
...
多层前馈神经网络恰好可以看做一个非线性函数 $\phi(\cdot)$ ，将输入 $\mathbf{x}\in \R^{1\times d}$ 映射到输出 $\phi(x)\in \R^{K}$ ，因此可将多层前馈神经网络看作一种特殊的特征转换方法，其输出 $\phi(x)$ 作为分类器的输入
$$
...
$$
其中，$\mathbf{o}_{L}\in \R^{1\times K}$ 为第 $L$ 层神经网络的净输出， $\hat{\mathbf{y}}\in\R^{1\times K}$ 为第 $L$ 层神经网络的活性值，每一维分别表示不同类别标签的预测条件概率
...
$$
\begin{cases}
\mathbf{o}_{l}\in \R^{1\times M_{l}}\\
\mathbf{W}\in\R^{M_{l}\times M_{l-1}}\\
\mathbf{b}_l\in \R^{1\times M_l}
\end{cases}\Rightarrow \begin{cases}
\frac{\partial \mathbf{o}_l}{\partial \mathbf{W}_l}\in \R^{M_l\times M_l\times M_{l-1}}\\
\frac{\partial \mathbf{o}_l}{\partial \mathbf{b}_l}\in \R^{M_l\times M_l}
\end{cases}
$$
因 $\mathbf{o}_l=\mathbf{h}_{l-1}\mathbf{W}_l^T+\mathbf{b}_l$ 

- 计算 $\frac{\partial \mathbf{o}_l}{\partial \mathbf{W}_l}\in \R^{M_l\times 1}$ 
  $$
  \begin{aligned}
  ...
  所以有 $\frac{\partial \mathbf{o}_l}{\partial \mathbf{W}_l}\in \R^{M_l\times M_l\times M_{l-1}}$ 
  $$
...
- 计算 $\frac{\partial \mathbf{o}_l}{\partial \mathbf{b}_l}$ 
  ...
  \end{bmatrix}=\begin{bmatrix}0&\cdots&1&\cdots&0\end{bmatrix}\in \R^{1\times M_l}
  $$
  其中，$i\in [1,M_l]$ 
  $$
  ...
  \vdots&\ddots&\vdots&\ddots&\vdots\\
  0&\cdots&0&\cdots&1\\
  \end{bmatrix}=\mathbf{I}_{M_l}\in \R^{M_l\times M_l}
  $$
...
- $\frac{\partial \mathbf{h}_l}{\partial \mathbf{o}_l}\in \R^{M_l\times M_l}$ 
  $$
  \begin{aligned}
...
- 由于第 $l+1$ 层输入 $\mathbf{x}_{l+1}$ 为第 $l$ 层活性值 $\mathbf{h}_l$ ，$\mathbf{o}_{l+1}=\mathbf{x}_{l+1}\mathbf{W}_{l+1}^T+\mathbf{b}_{l+1}=\mathbf{h}_{l}\mathbf{W}_{l+1}^T+\mathbf{b}_{l+1}$ 
  $$
  \begin{aligned}
  \frac{\partial \mathbf{o}_{l+1}}{\partial \mathbf{h}_{l}}&=\frac{\partial \left(\mathbf{h}_{l}\mathbf{W}_{l+1}^T+\mathbf{b}_{l+1}\right)}{\partial \mathbf{h}_{l}}\in \R^{M_{l+1}\times M_{l}}\\
  &=\mathbf{W}_{l+1}
  \end{aligned}
  $$
...
&=\frac{\partial \ell}{\partial \mathbf{o}_{l+1}}\frac{\partial \mathbf{o}_{l+1}}{\partial \mathbf{h}_l}\frac{\partial \mathbf{h}_l}{\partial \mathbf{o}_l}\\
&=\left(\delta_{l+1}\mathbf{W}_{l+1}\right)\odot diag\left[\sigma'\left(o_l^{(i)}\right)\right]\in \R^{1\times M_l}
\end{aligned}
$$
