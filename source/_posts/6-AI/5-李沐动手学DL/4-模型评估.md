...
$$
\text{p\_f}_{200\times 20}\cdot \text{true\_w}_{1\times 20}=\text{labels}_{200\times 1}\\
...
对于有 $L$ 层的神经网络，第 $l$ 层的活性值为 $\mathbf{h}^l$  
$$
\mathbf{h_l}=\sigma\left(\mathbf{o}_l\right)=\sigma(\mathbf{h}_{l-1}\mathbf{W}_l+\mathbf{b}_l)=f_l(\mathbf{h}_{l-1})
$$
其中，$\mathbf{h}_{l-1}\in \R^{1\times M_{l-1}}$ ，$\mathbf{W}\in\R^{M_{l-1}\times M_l}$ ，$\mathbf{b}_l\in \R^{1\times M_l}$ 
...
- $\frac{\partial\ell}{\partial\mathbf{W}_l}\in \R^{M_{l-1}\times M_l}$ ， $\frac{\partial\mathbf{h}_{l}}{\partial\mathbf{h}_{l-1}}\in\R^{M_{l}\times M_{l-1}}$ ，$\frac{\partial\mathbf{h}_{l}}{\partial\mathbf{W}_l}\in \R^{M_l\times M_{l-1}\times M_l}$ 
...
- $$
  ...
  &=diag\left[\sigma'\left(\mathbf{h}_{l-1}\mathbf{W}_l+\mathbf{b}_l\right)\right]
  \end{aligned}
  $$

- $$
  \begin{aligned}
  \frac{\partial \mathbf{o}_l}{\partial \mathbf{h}_{l-1}}&=\frac{\partial \left(\mathbf{h}_{l-1}\mathbf{W}_l+\mathbf{b}_l\right)}{\partial \mathbf{h}_{l-1}}\in \R^{M_l\times M_{l-1}}\\
  &=\mathbf{W}_l^T
  \end{aligned}
  $$

则 $\frac{\partial \mathbf{h_l}}{\partial \mathbf{h}_{l-1}}=diag\left[\sigma'\left(\mathbf{h}_{l-1}\mathbf{W}_l+\mathbf{b}_l\right)\right]\mathbf{W}_l^T$

故
$$
\frac{\partial\mathbf{h}_{L}}{\partial\mathbf{h}_{L-1}}\cdots\frac{\partial\mathbf{h}_{l+1}}{\partial\mathbf{h}_{l}}=\prod\limits_{i=l}^{L-1}\frac{\partial\mathbf{h}_{i+1}}{\partial\mathbf{h}_{i}}=\prod\limits_{i=l}^{L-1} diag\left[\sigma'\left(\mathbf{h}_{i-1}\mathbf{W}_i+\mathbf{b}_i\right)\right]\mathbf{W}_i^T
$$
...
> 0&,其他\end{cases}
> $$
> 此时，$\prod\limits_{i=l}^{L-1}\frac{\partial\mathbf{h}_{i+1}}{\partial\mathbf{h}_{i}}=\prod\limits_{i=l}^{L-1} diag\left[\sigma'\left(\mathbf{h}_{i-1}\mathbf{W}_i+\mathbf{b}_i\right)\right]\mathbf{W}_i^T$ 的一些元素会来自于 $\prod\limits_{i=l}^{L-1}\mathbf{W}_i^T$ ，若网络层数很深 $L-l-1$ 很大，则这些值可能非常大
...
>
> 此时，$\prod\limits_{i=l}^{L-1}\frac{\partial\mathbf{h}_{i+1}}{\partial\mathbf{h}_{i}}=\prod\limits_{i=l}^{L-1} diag\left[\sigma'\left(\mathbf{h}_{i-1}\mathbf{W}_i+\mathbf{b}_i\right)\right]\mathbf{W}_i^T$ 的元素值是 $L-l-1$ 个小数值的乘积
...
假设第 $l$ 层的网络参数 $\mathbf{W}_l^{(i,j)}$ 是独立同分布的，且 $E[w_l^{(i,j)}]=0,var[w_l^{(i,j)}]=\gamma_l$ 

第 $l-1$ 层的活性值 $\mathbf{h}_{l-1}$ 即第 $l$ 层的输入独立于第 $l$ 层的参数 $\mathbf{W}_l$ ，即 $h_{l-1}^{(i,j)}$ 独立于 $w_l^{(i,j)}$ 

正向均值
$$
E\left[h_l^{(i)}\right]=E\left[\sum\limits_{j=1}^{M_{l-1}}w_l^{(j,i)}h^{(j)}_{l-1}\right]=\sum\limits_{j=1}^{M_{l-1}}E\left[w_l^{(j,i)}\right]E\left[h^{(j)}_{l-1}\right]=0
$$
正向方差
$$
\begin{aligned}
var\left[h_l^{(i)}\right]&=E\left[\left(h_l^{(i)}\right)^2\right]-E\left[h_l^{(i)}\right]^2\\
&=E\left[\left(h_l^{(i)}\right)^2\right]=E\left[\left(\sum\limits_{j=1}^{M_{l-1}}w_l^{(j,i)}h^{(j)}_{l-1}\right)^2\right]\\
&=E\left[\sum\limits_{j=1}^{M_{l-1}}\left(w_l^{(j,i)}\right)^2\left(h^{(j)}_{l-1}\right)^2+\sum\limits_{j\neq k}^{M_{l-1}}w_l^{(j,i)}w_l^{(k,i)}h^{(j)}_{l-1}h^{(k)}_{l-1}\right]\\
&=\sum\limits_{j=1}^{M_{l-1}}E\left[\left(w_l^{(j,i)}\right)^2\right]E\left[\left(h^{(j)}_{l-1}\right)^2\right]\\
&=\sum\limits_{j=1}^{M_{l-1}}var\left[w_l^{(j,i)}\right]var\left[h_{l-1}^{(j)}\right]=M_{l-1}\gamma_lvar\left[h_{l-1}^{(j)}\right]
\end{aligned}
$$
