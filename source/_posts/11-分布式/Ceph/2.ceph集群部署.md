...
> monitor 不参与数据存储，不频繁利用内存，占用资源较少

**硬件需求**

- 若 **日志信息** 存储在本地monitor节点，就要确保monitor节点上有 **足够的磁盘空间** 来存放日志文件
- 网络级别的冗余对于monitor很重要，因为monitor节点间需要进行仲裁
	硬件需求：1Gbit/s的双网卡

#### OSD

> OSD是Ceph 中主要存储单元，需要配置足够的硬盘

**硬件需求**

- CPU(1GHz)和内存(2GB)：建议在早期多配置内存和CPU资源，因为在JBOD系统中，可以随时为主机增加硬盘
- OSD日志(块设备或文件)
- 基础文件系统(ext4，xfs，Btrfs)
- 独立的OSD或冗余的集群网络(推荐)
...
- 应该为每一个OSD配置独立的日志盘：采用SSD 盘的分区存储日志，机械盘存储数据
	兼顾成本与性能，每个SSD盘上可作为2～4个OSD日志盘

#### 网络需求

>两个独立网络：一个用于前端数据网络(公共网络)，一个用于后端数据网络(集群网络)
> 保证隔离客户度啊数据流量以及集群内部流量

![image-20231020195606317](2.ceph集群部署/image-20231020195606317.png)

前端数据网络用于客户端和Cep集群之间的连接
...
**最佳实践**
- 带宽：至少支持1Gb带宽，若考虑到未来的纵向扩展带来的工作负载(ROI/性能)，建议为数据和集群管理分别采用10Gbit/s的物理隔离网络
...
#2. Ceph OSD基于GUID分区表(GPT)工作，需要将分区表计改为GPT
...
确保为monitor的6789端口添加防火墙策略，若只是测试环境，可以关闭防火墙
