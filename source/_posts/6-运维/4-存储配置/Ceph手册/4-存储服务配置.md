...
2. 创建元数据池与数据池
   ...
   ceph osd pool application enable ceph-data nfs
   ceph osd pool application enable ceph-metadata nfs
   ```

3. 创建CephFS并验证

   ```shell
   [root@tzj ceph-cluster]# ceph fs new cephfs ceph_metadata ceph_data
