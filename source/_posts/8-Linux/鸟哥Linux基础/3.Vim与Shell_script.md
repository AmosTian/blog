...
- 有无 `:` ，仅会反转旧变量为空字符串的赋值
- `-` 模式：旧变量不存在，则新变量设为字符串；旧变量存在，则新变量设为旧变量的值
...
- 直接下达指令（脚本需要有执行权限x）
  ...
  - 变量：将 shell.sh 放到 `$PATH` 指定的目录内
- 以 bash 程序执行（脚本需要有读取权限r）
...
bash shell随执行脚本输入的变量，在shell 脚本中，会赋值给 `$1` 、`$2`、...
...
| 选项              | 作用                                            |
| ----------------- | ----------------------------------------------- |
| test -z string    | 判断string是否为0，若string为空字符串，则为true |
| test -n string    | 判断string是否为非0，空则为false                |
| test str1 == str2 | 是否相等                                        |
| test str1 != str2 | 是否不等                                        |
...
```shell
-a 两个判断同时成立，才返回true(and)
eg:
test -r file -a -x file
file同时具有r与x权限才会回传true
```

```shell
-o 两个判断一个成立，返回true(or)
eg:
...
对于多个条件判断组合的条件判断式，可依逻辑关系拆分为多个条件判断式
...
```shell
[dmtsai@study bin]$ cp ans_yn.sh ans_yn-3.sh
...
3. 而如果加入的参数不是hello ，就提醒使用者仅能使用hello 为参数。

```shell
tzj@LAPTOP-070162A7:~$ vim hello.sh
#!/bin/bash
...
tzj@LAPTOP-070162A7:~$ chmod +x hello.sh
tzj@LAPTOP-070162A7:~$ ./hello.sh
You MUST input parameters, ex> {./hello.sh someword}
tzj@LAPTOP-070162A7:~$ ./hello.sh hello
Hello, how are you ?
tzj@LAPTOP-070162A7:~$ ./hello.sh ajk
...
```shell
# 当逻辑判断式condition满足，才会执行 程序段落
while [ condition ]
do
	程序段落
done

# 当逻辑判断式 condition 满足，将不再执行 程序段落
until [ condition ]
do
	程序段落
done
...
# 提取本机账户名，然后使用 id 检查使用者的标识符与特殊参数
