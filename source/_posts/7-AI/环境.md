...
conda config --set envs_dirs D:\anaconda3\envs
...
为避免出上述错误，恢复默认源即可，官方默认源稳定，添加的其他源容易造成各种奇怪错误

```shell
conda config --remove-key channels
```

简单来说是求解器问题：https://stackoverflow.com/questions/74781771/how-we-can-resolve-solving-environment-failed-with-initial-frozen-solve-retry

当前环境下安装 `conda-libmamba-solver` 
...
使用mamba代替conda：Mamba是一个由conda社区开发的，与conda兼容的包管理器。它在处理包依赖关系及冲突问题时，通常会比conda更有优势。

```shell
conda install mamba -c conda-forge --solver=libmamba
```
...
https://www.cnblogs.com/corrschi/p/18084078
