...
1.  ServletContextListener:实现ServletContext的启动和销毁监听

    | contextDestoryed()方法   | 销毁ServletContext时触发该方法 |
    | ------------------------ | ------------------------------ |
    | contextInitialized()方法 | 创建ServletContext时触发       |

2.  ServletContextAttributeListener:实现application范围属性的变化监听
...
3.  HttpSessionBindingEventListener:监听HttpSession对象的绑定状态

    | valueBound()   | 调用setAttribute()时触发    |
    | -------------- | --------------------------- |
    | valueUnbound() | 调用removeAttribute()时触发 |

4.  HttpSessionActivationlIstener:监听绑定在HttpSession对象中的JavaBean状态
...
1.  ServletRequestListener:用于监听ServletRequest对象的变化

    | requestDestoryed()   | 销毁ServletRequest对象   |
    | -------------------- | ------------------------ |
    | requestInitialized() | 初始化ServletRequest对象 |

2.  ServletRequestAttributeListener:用于监听ServletRequest对象属性的变化
