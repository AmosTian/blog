...
任一方阵，$A\in C^{n\times n}$ ，必存在U阵 $Q$ ，使 $Q^{-1}AQ=Q^HAQ=B$ ，$B=\left(
\begin{matrix}
\lambda_1&*&\cdots&*\\
0&\lambda_2&\cdots&*\\
\vdots&\vdots &\ddots&\vdots\\
...
### 1.12.1 Hermite分解定理(对角阵)

若 $A=A^H$ 是Hermite阵，则存在U阵 $Q$ ,使 $Q^{-1}AQ=Q^HAQ=\Lambda=\left(
\begin{matrix}
\lambda_1&\quad&\quad\\
\quad&\ddots&\quad\\
\quad&\quad&\lambda_n
