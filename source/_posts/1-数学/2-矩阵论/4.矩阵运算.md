...
![image-20221108160917055](4.矩阵运算/image-20221108160917055.png)
...
- $A^{+}$ 具有唯一性
  ...
  &有X=XAX\xlongequal{A=AYA(替换右侧X)}XAYAX=X(AY)^H(AX)^H=X[(AX)(AY)]^H\xlongequal{AXA=A}X(AY)^H=\\
  &XAY\xlongequal{AYA=A(消去左侧X)}XAYAY=(XA)^H(YA)^HY=[(YA)(XA)]^HA=(YA)^HY=Y\\
  &故可得A^+具有唯一性
  \end{aligned}
  $$
...
- 若A为列优阵($A^HA=I$) ，则 $A^+=A^H$
  ...
  若 $A^H$ 为列优阵(行优阵)($AA^H=I$) ，则  $A^+=A^H$
  
- 若A为正规阵，则 $A^+$ 也为正规阵

  有正规分解 $A=QDD^H$ ，则 $A^+=QD^+Q^H$
  
  有谱分解 $A=\lambda_1^+G_1+\cdots+\lambda_kG_k$
...
$$
\begin{aligned}
&若A为正规阵，且有谱分解：A=\lambda_1G_1+\cdots+\lambda_kG_k,则有 A^+=\lambda_1^+G_1+\lambda_2G_2+\cdots+\lambda_kG_k
\end{aligned}
$$
