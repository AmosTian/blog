...
class Utils():
    ...
    @staticmethod
    def tree_is_in_md_lists(tree, md_lists):
        if tree["content"] in md_lists:
            return True
        ...
        if os.path.isdir(path):
            ...
            for filename in os.listdir(path):
                ...
                if os.path.isdir(child) and \
                    ...
                    has_dirs = True
                if child.endswith(".md") and \
                    os.path.basename(child) not in jump_dir:
                    has_mds = True
                    md_lists.append(Utils.process_file(child))
            ...
            for subtree in tree["content"]:
                if "file" == subtree["type"] and \
                    "md_lists" in tree and \
                        Utils.tree_is_in_md_lists(subtree, tree["md_lists"]):
                    continue
...
if __name__ == "__main__":
    ...
    pre_content = pre_content_perfix + "updated: " + \
            str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")) + \
        pre_content_supfix
    
    with open(os.path.join(directory_path, "menu.md"), "w", encoding="utf-8") as f:
        f.write(pre_content)
